...
/// </returns>
int fineTuning(uint minK, uint maxK, const std::vector<TSample> &dataset, uint folds, int rank, int numprocs)
{
    ...
    for(int k = minK; k <= maxK; k++)
    {
        ...
        double accuracy = crossValidation.validate(k, rank, numprocs);
...
int main(int argc, char** argv) 
{
    ...
    double start = omp_get_wtime();
    int k = fineTuning(1, 10, std::vector<TSample>(train.begin(), train.begin() + 20000), 5, rank, numprocs);
    ...
    KNN knn(train, k);
    double accuracy = knn.classifyAndEvaluate(test, labels, rank, numprocs);
