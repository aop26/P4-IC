...
/// </returns>
double KNN::classifyAndEvaluate(const std::vector<TSample> &test, std::vector<int> &labels, int rank, int numprocs) const
{
    ...
    int matches = 0;
     if(rank==0){
        ...
        {
            int label = -1;
            MPI_Recv(&label,1,MPI_INT,0,1,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
            std::cout<<i<<"/"<<test.size()<<std::flush;
        ...
        for(int i=0;i<test.size();i++)
        {
            if((i%(numprocs-1)+1) != rank){
                continue;
            }
            //std::cout<<i<<"/"<<test.size()<<"\r"<<std::flush;
            int label = this->classify(test[i]);

            MPI_Send(&label,1,MPI_INT,0,1,MPI_COMM_WORLD);
            //labels.push_back(label);
